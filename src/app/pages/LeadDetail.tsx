...
export function LeadDetail() {
  ...
  const [contractCreateVisible, setContractCreateVisible] = useState(false);
  const [contractForm] = Form.useForm();
  const [paymentPlanCount, setPaymentPlanCount] = useState(2);
  const [paymentPlans, setPaymentPlans] = useState([
    { period: 1, expectedDate: '', amount: 0, percentage: 0 },
    { period: 2, expectedDate: '', amount: 0, percentage: 0 },
  ]);
  ...
  // 处理回款计划期数变化
  const handlePaymentPlanCountChange = (count: number) => {
    setPaymentPlanCount(count);
    const newPlans = Array.from({ length: count }, (_, index) => ({
      period: index + 1,
      expectedDate: paymentPlans[index]?.expectedDate || '',
      amount: paymentPlans[index]?.amount || 0,
      percentage: paymentPlans[index]?.percentage || 0,
    }));
    setPaymentPlans(newPlans);
  };

  // 更新回款计划金额
  const handlePaymentPlanAmountChange = (index: number, amount: number) => {
    const newPlans = [...paymentPlans];
    newPlans[index].amount = amount;

    const totalAmount = contractForm.getFieldValue('totalAmount') || 0;
    if (totalAmount > 0) {
      newPlans[index].percentage = Math.round((amount / totalAmount) * 10000) / 100;
    }
  ...
  const handleGenerateContractPreview = () => {
    contractForm.validate().then(() => {
      const totalAmount = contractForm.getFieldValue('totalAmount') || 0;
      const planTotal = paymentPlans.reduce((sum, plan) => sum + plan.amount, 0);

      if (Math.abs(planTotal - totalAmount) > 0.01) {
        Message.error(`回款计划总额（¥${planTotal}）与合同总额（¥${totalAmount}）不一致，请调整`);
        return;
      }
...
{item.approvalFlow.map((node, index) => (
              ...
              <Card
                ...
                extra={
                  <Button
                    ...
                    icon={<IconPlus />}
                    size="small"
                    onClick={() => setContractCreateVisible(true)}
                  >
                    合同
                  </Button>
                }
      ...
      <Modal
        title="创建合同"
        visible={contractCreateVisible}
        onCancel={() => {
          setContractCreateVisible(false);
          contractForm.resetFields();
          setPaymentPlanCount(2);
          setPaymentPlans([
            { period: 1, expectedDate: '', amount: 0, percentage: 0 },
            { period: 2, expectedDate: '', amount: 0, percentage: 0 },
          ]);
        ...
        footer={
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Button
              ...
              disabled={contractPreviewVisible}
            >
              ⚙️ 生成电子版合同
            </Button>
            <div>
              <Button onClick={() => setContractCreateVisible(false)}>取消</Button>
              <Button
                type="primary"
                onClick={() => {
                  contractForm.validate().then(() => {
        ...
        style={{ width: 1000, maxWidth: '90vw' }}
      >
        <Form form={contractForm} layout="vertical" autoComplete="off">
          {/* 基础信息区 */}
          <div style={{
            background: 'var(--color-fill-2)',
            padding: '16px',
            ...
            <div style={{ fontWeight: 600, marginBottom: 12, fontSize: 14 }}>基础信息</div>
            <Grid.Row gutter={16}>
              <Grid.Col span={12}>
                <FormItem
                  label="合同编号"
                  tooltip="系统自动生成，作废即失活"
                >
                  <Input
                    disabled
                    value={`CT${new Date().toISOString().slice(0, 10).replace(/-/g, '')}001`}
            ...
            <Grid.Row gutter={16}>
              <Grid.Col span={12}>
                ...
                >
                  <Select placeholder="请选择">
                    <Select.Option key="product-1" value="软件开发">软件开发</Select.Option>
                    <Select.Option key="product-2" value="系统集成">系统集成</Select.Option>
                    <Select.Option key="product-3" value="技术服务">技术服务</Select.Option>
                    <Select.Option key="product-4" value="云服务">云服务</Select.Option>
                  </Select>
                </FormItem>
              </Grid.Col>
              <Grid.Col span={12}>
                <FormItem label="签约主体">
                  <Input disabled value="北京科技有限公司" style={{ background: 'var(--color-fill-3)' }} />
                </FormItem>
              </Grid.Col>
            </Grid.Row>
            ...
            <FormItem label="数据来源">
              <div style={{
                ...
                关联自线索 <a style={{ fontWeight: 600 }}>某科技公司APP开发需求</a> 及报价单 <a style={{ fontWeight: 600 }}>QT20260425001</a>
              </div>
            </FormItem>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: 12, fontSize: 14 }}>甲方（客户）信息</div>
            <Grid.Row gutter={16}>
              <Grid.Col span={12}>
                <FormItem
                  label="公司名称"
                ...
                >
                  <Input placeholder="自动从客户信息抓取" defaultValue="北京科技有限公司" />
                </FormItem>
              </Grid.Col>
              <Grid.Col span={12}>
                <FormItem
            ...
            <Grid.Row gutter={16}>
              <Grid.Col span={12}>
                ...
                >
                  <Input placeholder="请输入联系电话" defaultValue="13800138000" />
                </FormItem>
              </Grid.Col>
              <Grid.Col span={12}>
                <FormItem label="电子邮件" field="customerEmail">
                  <Input placeholder="请输入电子邮件" />
                </FormItem>
              </Grid.Col>
            </Grid.Row>

            <FormItem label="通讯地址" field="customerAddress">
              <Input.TextArea placeholder="请输入通讯地址" rows={2} />
            </FormItem>

            <div style={{ fontWeight: 600, marginTop: 16, marginBottom: 12, fontSize: 13 }}>开票信息</div>
            <Grid.Row gutter={16}>
              <Grid.Col span={12}>
                <FormItem label="汇款人" field="payerName">
                  <Input placeholder="请输入汇款人" />
                </FormItem>
              </Grid.Col>
              <Grid.Col span={12}>
            ...
            <Grid.Row gutter={16}>
              ...
              <Grid.Col span={12}>
                <FormItem label="税务登记号" field="taxNumber">
          ...
          <div style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: 12, fontSize: 14 }}>合同核心条款</div>
            ...
            >
              <Select placeholder="请选择">
                <Select.Option key="payment-1" value="公对公">公对公</Select.Option>
                <Select.Option key="payment-2" value="私对公">私对公</Select.Option>
                <Select.Option key="payment-3" value="分期付款">分期付款</Select.Option>
              </Select>
            </FormItem>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: 12, fontSize: 14 }}>财务总览</div>
            ...
            <Grid.Row gutter={16}>
              <Grid.Col span={12}>
                ...
                >
                  <Input
                    type="number"
                    placeholder="从报价单自动获取"
                    defaultValue={500000}
                    onChange={(value) => {
                      const amount = parseFloat(value) || 0;
                      contractForm.setFieldValue('totalAmountChinese', convertToChinese(amount));
                    }}
                  />
                </FormItem>
              </Grid.Col>
              <Grid.Col span={12}>
                <FormItem label="大写金额" field="totalAmountChinese">
                  <Input
                    disabled
                    style={{ background: 'var(--color-fill-3)', color: 'rgb(var(--primary-6))', fontWeight: 600 }}
                    placeholder="系统自动转换"
                    value={contractForm.getFieldValue('totalAmount') ? convertToChinese(contractForm.getFieldValue('totalAmount')) : ''}
                  />
                </FormItem>
              </Grid.Col>
            </Grid.Row>

            <FormItem label="返点金额（元）" field="rebateAmount">
              <Input type="number" placeholder="请输入返点金额（选填）" />
            </FormItem>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 13, color: 'var(--color-text-3)' }}>付款期数：</span>
                <Input
                  type="number"
                  min={1}
                  max={10}
                  value={paymentPlanCount}
                  onChange={(value) => handlePaymentPlanCountChange(parseInt(value) || 2)}
                  style={{ width: 80 }}
                />
              </div>
            </div>
            ...
            <div style={{
              ...
              {paymentPlans.map((plan, index) => (
                <div key={index} style={{
                  ...
                  gap: 12,
                  alignItems: 'center',
                  marginBottom: index < paymentPlans.length - 1 ? 12 : 0,
                  padding: '8px',
                  ...
                  <Input
                    ...
                    onChange={(value) => {
                      const newPlans = [...paymentPlans];
                      newPlans[index].expectedDate = value;
                      setPaymentPlans(newPlans);
                    }}
                  />
                  <Input
                    ...
                    placeholder="金额（元）"
                    value={plan.amount || ''}
                    onChange={(value) => handlePaymentPlanAmountChange(index, parseFloat(value) || 0)}
                  />
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'rgb(var(--primary-6))' }}>
              ...
              <div style={{
                ...
                <span style={{ fontSize: 13, fontWeight: 600, color: '#003a8c' }}>合计：</span>
                <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: 'rgb(var(--primary-6))' }}>
                    ¥{paymentPlans.reduce((sum, plan) => sum + plan.amount, 0).toLocaleString()}
                  </span>
                  <span style={{ fontSize: 13, color: 'var(--color-text-3)' }}>
                    {paymentPlans.reduce((sum, plan) => sum + plan.percentage, 0).toFixed(2)}%
                  </span>
                </div>
              </div>
            </div>
          ...
          <div style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: 12, fontSize: 14 }}>合同模板与电子版生成</div>
            ...
            >
              <Select placeholder="根据产品类别自动过滤">
                <Select.Option key="template-1" value="software_sales">软件销售合同模板</Select.Option>
                <Select.Option key="template-2" value="service_contract">技术服务合同模板</Select.Option>
                <Select.Option key="template-3" value="cloud_service">云服务合同模板</Select.Option>
              </Select>
            </FormItem>

            {contractPreviewVisible && (
              <div style={{
                marginTop: 16,
                ...
                <div style={{
                  ...
                  <span style={{ fontWeight: 600 }}>📄 合同预览</span>
                  <Space>
                    <Button key="download-pdf" size="small" onClick={() => Message.info('开始下载 PDF')}>
                      📥 下载 PDF
                    </Button>
                    <Button key="download-word" size="small" onClick={() => Message.info('开始下载 Word')}>
                      📥 下载 Word
                    </Button>
                  </Space>
                </div>
                <div style={{
                  ...
                  <div style={{ textAlign: 'center', marginBottom: 30 }}>
                    <div style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>
                      {contractForm.getFieldValue('contractName') || '合同名称'}
                    </div>
                    <div style={{ fontSize: 14, color: 'var(--color-text-3)' }}>
                      合同编号：CT{new Date().toISOString().slice(0, 10).replace(/-/g, '')}001
                    </div>
                  </div>

                  <div style={{ fontSize: 14, lineHeight: 2, color: 'var(--color-text-1)' }}>
                    <p><strong>甲方（客户）：</strong><span style={{ background: '#e6f7ff', padding: '2px 6px' }}>{contractForm.getFieldValue('customerName') || '___________'}</span></p>
                    <p><strong>乙方（我方）：</strong><span style={{ background: '#e6f7ff', padding: '2px 6px' }}>北京科技有限公司</span></p>
                    <p><strong>合同金额：</strong><span style={{ background: '#fffbe6', padding: '2px 6px', fontWeight: 600 }}>¥{contractForm.getFieldValue('totalAmount')?.toLocaleString() || '0'} ({convertToChinese(contractForm.getFieldValue('totalAmount') || 0)})</span></p>
                    <p><strong>签约日期：</strong>{contractForm.getFieldValue('signDate') || '___________'}</p>
                    <p><strong>生效日期：</strong>{contractForm.getFieldValue('effectiveDate') || '___________'}</p>
                    <p style={{ marginTop: 20 }}>
                      <strong>合同内容：</strong><br />
                      <span style={{ background: '#f0f0f0', padding: '8px 12px', display: 'block', marginTop: 8, borderRadius: 4 }}>
                        {contractForm.getFieldValue('contractContent') || '（待填写）'}
                      </span>
                    </p>
                    <p style={{ marginTop: 20, fontSize: 13, color: 'var(--color-text-3)', fontStyle: 'italic' }}>
                      注：以上为合同预览，高亮部分为系统自动填充的变量。如发现信息有误，请返回表单修改后重新生成。
                    </p>
                  </div>
                </div>
              </div>
